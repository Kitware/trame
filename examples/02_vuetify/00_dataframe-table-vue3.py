...
class DataframeTable(TrameApp):
    ...
    def configure_table(self):
        ...
        self.state.setdefault("group_by", [{"key": "glutenfree", "order": "asc"}])

        # fmt: on
        _, self.rows = v3.dataframe_to_grid(self.frame)

        self.table = {
            "group_by": ("group_by",),
            "headers": ("headers", self.headers),
            "items": ("rows", self.rows),
            "search": ("query", ""),
    ...
    @change("rows")
    def update_values(self, rows, **kwargs):
        print("rows changed")
                    ...
                    with v3.Template(
                        ...
                        with html.Tr():
                            with html.Td(colspan=("columns.length",)):
                                ...
                                html.Span(
                                    "{{ item.value ?'Gluten free' : 'Contains gluten' }}"
                                )
